...
def discover(logger=None, package=DETECTOR_PACKAGE):
    """
    ...
    try:
        ...
        if logger:
            logger.warn(f"未找到检测器包 '{package}'，仅使用魔数表。")
        return []
    ...
    try:
        ...
        if logger:
            logger.warn(f"未找到 {package}.base，插件系统不可用。")
        return []
    ...
    for _, mod_name, _ in pkgutil.iter_modules(pkg.__path__):
        ...
        try:
            ...
            if logger:
                logger.warn(f"加载检测器 '{mod_name}' 失败: {e}")
            continue
        ...
        for _, obj in inspect.getmembers(mod, inspect.isclass):
            if (issubclass(obj, Base)
                ...
                if logger:
                    logger.notice(f"已加载检测器: {obj.name} -> {obj.target_extensions}")
